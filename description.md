...
**Useful Kuwait Websites.** Moved here from the Official Search Engine page in the Sept 2026 UI overhaul (`js/kuwait-websites.js`, imported by `dashboard.js`). Sits below Critical Upcoming Hearings: cover-image cards linking to the official Kuwaiti authorities (Ministry of Justice, E-Services of MOJ, PACI, Civil Service Commission), each with what it offers and why it's useful. Links open in a new tab. Note: `tawtheeq.moj.gov.kw` does not resolve (DNS NXDOMAIN), so this entry is linked through the verified `https://eservices.moj.gov.kw` portal instead.

### 3.6a' Dashboard upgrade — dense, priority-first, 2-column layout (Sub-phase 3.4, Sept 2026 overhaul)

The Dashboard's stat grids and the Quick Case Access panel keep their existing full-width position at the top of the page. Below them, a new `.dash-grid` two-column region (collapsing to one column at ≤1024px, per the app's documented breakpoint reference in `css/styles.css`) holds five widgets, all in `js/pages/dashboard.js` unless noted:
