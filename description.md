...
**One sidebar entry, two tabs (Merge A, Sept 2026 overhaul).** "Cases" is a single `ROUTES` entry rendered by `js/pages/cases-hub.js`, which mounts two existing page modules unchanged into contained tabs: **Cases** (`js/pages/cases.js`; labelled *Cases* until Sept 2026) and **Official Search Engine** (`js/pages/search.js`, section 3.1 above). The hub is its own module because `search.js` already imports `openCaseDetail` from `cases.js`; `cases.js` importing `search.js` back would be circular. All the hub *behavior* below lives in the shared `js/hub.js::createHub`, which the Notifications hub (section 3.4) is built on too; `cases-hub.js` itself is only configuration.

- **Permission gate.** `visibleSections()` keeps only the tabs whose permission key (`cases`, `search`) is not `none`. A Client's `search` is `none`, so a Client gets Cases alone and **no tab bar is rendered at all**; the server independently refuses every `/api/search/*` call for that role regardless.
- **URL.** `#/cases` is Cases, `#/cases/search` the search tab. Any other second segment (`#/cases/active`, `#/cases/today-hearings`) is a Cases filter read by `cases.js` itself, and is remembered so leaving the search tab and coming back restores the filtered list. Tab switches update the URL with `history.replaceState` (no history entry, no re-render). The old `#/search` route is rewritten to `#/cases/search` by `ROUTE_ALIASES` in `js/app.js`, so bookmarks keep working; a Client following such a link lands on Cases with the segment dropped.
...
**The Cases tab** (page heading *Cases & Case Statements*). The case list is a **table** with two dropdown filters above it — **Status** (All/Active/Closed) and **Assigned Lawyer**. Arriving from a Dashboard stat card pre-applies the matching filter and shows a dismissible filter chip. There is no Kanban/board view.
...
Found under **"Notifications"** in the sidebar, as that page's second tab, **Reminders & Follow-ups** (the two used to be separate sidebar entries). This is how staff nudge each other about a case without a side conversation getting lost.

**One sidebar entry, two tabs (Merge B, Sept 2026 overhaul).** "Notifications" is a single `ROUTES` entry rendered by `js/pages/notifications-hub.js`, which mounts `js/pages/notifications.js` and `js/pages/reminders.js` unchanged as contained tabs. It is configuration on the same `js/hub.js::createHub` as the Cases hub (section 3.2), so the permission gate, URL handling, lazy mounting, tab-click refresh and keyboard behavior are identical — see 3.2 for the details, which are not repeated here. Specific to this hub:

- **Clients.** A Client's `reminders` permission is `none`: they get the Notifications tab alone and no tab bar. `reminders.js` is never mounted for them, so **no `/api/reminders` request is ever sent** for that role; the server refuses it anyway (`require_module("reminders")` on every reminders endpoint).
- **URL.** `#/notifications` is the Notifications tab; `#/notifications/reminders[/<filter>]` the Reminders & Follow-ups tab (`mine-open`, `overdue`, `status-requests`, `assigned-by-me`). Old `#/reminders[/<filter>]` links are rewritten there by `ROUTE_ALIASES`, filter kept. `ui.js::routeFilter()` knows the hub tab segments (`HUB_TAB_SEGMENTS`) so `reminders.js` still reads its filter from the segment after `reminders`. The Dashboard's four Tasks & Assignments cards, its Assign a Task quick action and its Quick search entry point at the new paths.
- **Caches.** `api.js::listNotificationsCached` / `listRemindersCached`, same 60s semantics as the case list. Every call that changes them invalidates them (mark read / mark all read; create / resolve reminder; Request Update) — resolving also invalidates the case list, because marking a status-update request Done advances the case stage.
- **Print.** Each tab keeps its own Print button (notification history; task list as filtered), unchanged.
- **Label.** The bottom-nav short label is now "Notifications" too (it read "Alerts"), so the page has one name everywhere.
...
- **Keyboard and screen-reader access (Sept 2026 audit).** Every interactive element on every page and modal has an accessible name (audited live with a script that resolves names the way the accessibility tree does). Controls that were mouse-only are now real controls: sidebar and phone "More" route entries are `<a href>` links with `aria-current="page"` on the active one (they were `<div>`s — the app could not be navigated from the sidebar by keyboard at all); the More sheet's language and Logout entries and Case Details' Watch toggle are `<button>`s; unread notification rows and both upload dropzones use `ui.js::makeKeyboardActivatable` (focusable, `role="button"`, Enter/Space) — the dropzones' file inputs are `hidden`, so before this a keyboard user could not upload a document. The bell and the bottom-nav Notifications item put the unread count in their accessible name (their `aria-label` had hidden the visible badge number). Case table rows remain mouse-clickable only by design: each row has a real "Case Details" button. All focus indicators use a 2px outline at ≥3:1 (light accent on the dark sidebar).
...
4. Go to **"Notifications"** in the sidebar, then the **"Reminders & Follow-ups"** tab — your new reminder should be listed as "Open."
...
- **Navigation labels are content-aware:** the sidebar and the "More" sheet use full descriptive labels (`nav_documents` = "Document Center"); the bottom tab bar and the top bar's title use short labels (`nav_documents_short` = "Documents") via a shared `.label-full`/`.label-short` CSS pair that both markup locations render — the swap is a pure CSS breakpoint flip, never a JS viewport check.
