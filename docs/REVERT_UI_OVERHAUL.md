...
| 1 — Cases + Search merge | new `js/pages/cases-hub.js` and (follow-up commit) the shared `js/hub.js` it is built on, also used by the Notifications hub — revert that follow-up and Task 2 together or neither; `js/app.js` (`ROUTES`, `ROUTE_ALIASES`); `js/api.js` (list caches); `js/pages/cases.js` + `js/pages/search.js` (cache calls, `.search-actions` moved inside its grid — both files **kept**, mounted as tabs); `js/pages/dashboard.js` (`#/cases/search` links); `js/i18n.js`; `css/styles.css` (`.hub-*`, `.search-form-grid`); `GUIDELINES.md`; `description.md` | `git revert` the merge commit | Simpler than §4 anticipated: neither page file was deleted, so the revert only restores the `search` entry in `ROUTES` and removes the hub — no file resurrection involved. |
| 2 — Notifications + Reminders merge | new `js/pages/notifications-hub.js` (on the shared `js/hub.js`); `js/app.js` (`ROUTES`, `ROUTE_ALIASES`); `js/api.js` (notifications/reminders caches); `js/ui.js` (`routeFilter` hub segments); `js/pages/notifications.js` + `js/pages/reminders.js` (cache calls, new `#/notifications/reminders` link — both files **kept**, mounted as tabs); `js/pages/dashboard.js` (task card targets); `js/i18n.js`; `GUIDELINES.md`; `description.md` | `git revert` the merge commit | Same shape as Task 1: no file was deleted, so the revert restores the `reminders` entry in `ROUTES` and removes the hub. Ordering: `js/hub.js` came in with the Cases hub follow-up commit, and this hub imports it — so if both are being undone, revert **this** commit first, then that one. Reverting that one alone would leave this hub importing a deleted module. |
